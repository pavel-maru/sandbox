...
- **Многообразие задач** – от простых алгоритмов до работы с файлами и структурами данных.
- **Документированный код** – каждый файл содержит описание, аннотации типов и примеры использования.
- **Кроссплатформенность** – программы работают в любой среде с установленным Python 3 или компилятором C11.
- **Минимализм** – все программы используют только стандартные библиотеки, без внешних зависимостей.
- **Готовность к использованию** – каждая программа самодостаточна и не требует дополнительной настройки.
...
2. Убедитесь, что установлены необходимые компиляторы и интерпретаторы.

3. (Опционально) Для Python можно создать виртуальное окружение – но зависимости отсутствуют, поэтому это не обязательно.
...
Перейдите в папку `sandbox-python/` и выполните:

        python3 имя_файла.py

Например:

        python3 fizzbuzz_variants.py

### Запуск C-программ

Перейдите в папку `sandbox-c/` и скомпилируйте программу:
...
Например:
        ...
        gcc -std=c11 rad_to_deg.c -lm -o rad_to_deg
        ./rad_to_deg
...
- Все программы самодостаточны и не требуют внешних зависимостей (кроме стандартной библиотеки).
- Для Python-программ используется только встроенный модуль `sys`, `os` и т.п.
- Для C-программ используется только стандартная библиотека C (и `math.h` для `rad_to_deg.c`).
- Код оформлен в едином стиле: для Python – PEP 8, для C – K&R.
- В каждой программе есть обработка ошибок ввода и краевых случаев.
...
**pavel-maru** – [https://github.com/pavel-maru](https://github.com/pavel-maru)
